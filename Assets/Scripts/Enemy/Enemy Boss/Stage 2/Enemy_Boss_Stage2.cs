...
public class Enemy_Boss_Stage2 : Enemy_Base
{
    ...
    public GameObject[] attack_Warring;
    ...
    // Sound Index
    // 0 : Counter
    // 1 : Combo A B C, backstep Slash
    // 2 : Ground & Air Rush Charge
    // 3 : Ground & Air Rush
    // 4 : Super Charge
    // 5 : Super B
    // 6 : Explosion
    ...
    private void Think()
    {
        ...
        {
            guardCoroutine = StartCoroutine(CounterWait());
        }
    }
    ...
    private IEnumerator CounterWait()
    {
        ...
        if(isHit)
        {
            ...
            // Hit Attack
            hitStopCoroutine = StartCoroutine(CountSlash()); // 여기 이슈 있을듯? -> 코루틴 담겨진 상태에서 다시 담는 듯? -> 테스트 필요함
    ...
    private IEnumerator CountSlash()
    {
        ...
        CurTarget_Check();
        LookAt();
        transform.localScale = transform.localScale.x == -1 ? new Vector3(1, 1, 1) : new Vector3(-1, 1, 1);
    ...
    private IEnumerator GroundRush()
    {
        ...
        for (int i = 0; i < (isPhase2 ? 2 : 1); i++)
        {
            ...
            // Warring On
            attack_Warring[2].SetActive(true);
            ...
            // Warring Off
            attack_Warring[2].SetActive(false);
    ...
    private IEnumerator GroundRushMove()
    {
        ...
        // Move Pos Setting
        Vector2 rayDir = (groundRushPos.position - transform.position).normalized;
        RaycastHit2D hit = Physics2D.Raycast(transform.position, rayDir, 7, groundLayer);

        Vector3 startPos = transform.position;
        Vector3 endPos = !hit ? groundRushPos.position : hit.point;
        endPos.x += transform.localScale.x < 0 ? 0.5f : -0.5f;
    ...
    {
        comboCollider[index].SetActive(!comboCollider[index].activeSelf);
    }
    ...
    private IEnumerator BackstepSlash()
    {
        ...
        // Warring On
        attack_Warring[4].SetActive(true);


        // Move Setting
        Vector3 rayDir = (backstepPos.position - transform.position).normalized;
        RaycastHit2D wallHit = Physics2D.Raycast(transform.position, rayDir, 7f, groundLayer);
        Vector3 pos = wallHit.point;
        Vector3 startPos = transform.position;
        Vector3 endPos = wallHit ? pos : backstepPos.position;
        ...
        while (timer < 1)
        {
            ...
            // Wall Check
            RaycastHit2D wall = Physics2D.Raycast(transform.position, rayDir, 1f, groundLayer);
            if (!wall)
            {
                // Movement
                transform.position = Vector3.Lerp(startPos, endPos, EasingFunctions.OutExpo(timer));
    ...
    {
        // Warring Off
        attack_Warring[4].SetActive(false);

        // Collider On Off
        backstepcollider.SetActive(!backstepcollider.activeSelf);
    }
    ...
    private IEnumerator SuperSlash()
    {
        ...
        // Warring On
        attack_Warring[5].SetActive(true);
        ...
        // Warring Off
        attack_Warring[5].SetActive(false);
